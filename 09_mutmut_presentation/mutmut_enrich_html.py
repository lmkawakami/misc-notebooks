...
STYLES = "html{font-family:monospace;color:#ccd4db;background-color:#1f1d1d}table{padding:1em}td.type-count,th.type-count{border:1px solid #ddd;text-align:center;padding:8px;display:block}tr.type-count{float:left;display:block}a{color:#6289dc}a:visited{color:#ba66ea}.operator{color:#fa6060}.keyword{color:#faad60}.number{color:#fafa60}.name{color:#adfa60}.string{color:#60fa60}.argument{color:#60faad}.or_test{color:#60fafa}.and_test{color:#60adfa}.lambdef{color:#6060fa}.expr_stmt{color:#ad60fa}.decorator{color:#fa60fa}.annassign{color:#fa60ad}"
...
def link_stylesheet(soup):
    ...
    head = BeautifulSoup(f'<head><style>{STYLES}</style></head>')
    body = soup.find('body')
...
for _path, muts in mut_tree.items():
    ...
    for h3 in soup.find_all('h3'):
        ...
        try:
            ...
            continue
        print(f"🔴mutant number {_index} from {muts}")
        _type = muts[_index]
...
soup = BeautifulSoup(f'<html><head><style>{STYLES}</style></head></html>')
